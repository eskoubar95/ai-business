...
| Migration | Purpose |
| --- | --- |
| **`0003_harsh_turbo.sql`** | DDL: enums (`task_status`, `task_log_author_type`), new tables (`user_settings`, `agent_archetypes`, `agent_documents`, `skill_files`, `agent_mcp_access`, `tasks`, `task_logs`), business columns (`description`, `github_repo_url`, `local_path`), `agents.archetype_id`, nullable `mcp_credentials.business_id`. Then **data backfill**: seed archetypes (`vertical-fullstack`, `harness-engineer`), copy `agents.instructions` → `agent_documents` (`soul`/`tools`/`heartbeat`), copy `skills.markdown` → `skill_files` (`SKILL.md`), set MCP `business_id`, populate `agent_mcp_access`. |
