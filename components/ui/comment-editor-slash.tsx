...
const SlashCommandList = forwardRef<SlashCommandListHandle, SlashCommandListProps>(
  ({ items, command }, ref) => {
    ...
    return (
      <div className="z-50 w-[220px] overflow-hidden rounded-lg border border-white/[0.10] bg-[#181818] shadow-2xl shadow-black/60">
        {items.length === 0 ? (
...
export function createSlashExtension(_editorRef: MutableRefObject<ReturnType<typeof useEditor>>) {
      ...
      return [
        Suggestion({
          ...
          items: ({ query }: { query: string }) =>
            SLASH_COMMANDS.filter((c) => c.title.toLowerCase().startsWith(query.toLowerCase())),
          render: () => {
